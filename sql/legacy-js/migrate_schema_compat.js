...
async function migrate() {
  ...
  await addColumnIfMissing({
    tableName: 'user',
    columnName: 'username',
    addSql: 'ALTER TABLE user ADD COLUMN username VARCHAR(50) NULL DEFAULT NULL'
  });
  // username 唯一索引（允许多个 NULL）
  await addIndexIfMissing({
    tableName: 'user',
    indexName: 'uk_username',
    addSql: 'CREATE UNIQUE INDEX uk_username ON user (username)'
  });
  await addColumnIfMissing({
    tableName: 'user',
    columnName: 'account',
    addSql: 'ALTER TABLE user ADD COLUMN account VARCHAR(11) NULL DEFAULT NULL'
  });
  // 账号唯一索引（允许多个 NULL）
  await addIndexIfMissing({
    tableName: 'user',
    indexName: 'uk_account',
    addSql: 'CREATE UNIQUE INDEX uk_account ON user (account)'
  });
  await addIndexIfMissing({
    tableName: 'user',
    indexName: 'idx_phone',
